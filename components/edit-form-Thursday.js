...
const EditFormThursday = ({ defaultValues, id, token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation UpdateAThursday($id: ID!, $event: String!) {
        updateThursday(id: $id, data: { event: $event }) {
          event
        }
      }
    `;
...
export default EditFormThursday;
