...
const EditFormSaturday = ({ defaultValues, id, token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation UpdateASaturday($id: ID!, $event: String!) {
        updateSaturday(id: $id, data: { event: $event }) {
          event
        }
      }
    `;
...
export default EditFormSaturday;
