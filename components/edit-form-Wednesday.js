...
const EditFormWednesday = ({ defaultValues, id, token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation UpdateAWednesday($id: ID!, $event: String!) {
        updateWednesday(id: $id, data: { event: $event }) {
          event
        }
      }
    `;
...
export default EditFormWednesday;
