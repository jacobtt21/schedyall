...
const NewThursday = ({ token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation CreateAThursday($event: String!, $owner: ID!) {
        createThursday(data: { event: $event, owner: { connect: $owner } }) {
          event
          owner {
            _id
          }
