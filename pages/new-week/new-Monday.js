...
const NewMonday = ({ token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation CreateAMonday($event: String!, $owner: ID!) {
        createMonday(data: { event: $event, owner: { connect: $owner } }) {
          event
          owner {
            _id
          }
