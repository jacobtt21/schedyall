...
const NewFriday = ({ token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation CreateAFriday($event: String!, $owner: ID!) {
        createFriday(data: { event: $event, owner: { connect: $owner } }) {
          event
          owner {
            _id
          }
