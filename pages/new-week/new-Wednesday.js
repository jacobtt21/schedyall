...
const NewWednesday = ({ token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation CreateAWednesday($event: String!, $owner: ID!) {
        createWednesday(data: { event: $event, owner: { connect: $owner } }) {
          event
          owner {
            _id
          }
