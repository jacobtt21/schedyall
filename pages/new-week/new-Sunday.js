...
const NewSunday = ({ token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation CreateASunday($event: String!, $owner: ID!) {
        createSunday(data: { event: $event, owner: { connect: $owner } }) {
          event
          owner {
            _id
          }
