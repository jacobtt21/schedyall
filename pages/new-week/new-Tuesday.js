...
const NewTuesday = ({ token }) => {
  ...
  const onSubmit = handleSubmit(async ({ event }) => {
    ...
    const mutation = gql`
      mutation CreateATuesday($event: String!, $owner: ID!) {
        createTuesday(data: { event: $event, owner: { connect: $owner } }) {
          event
          owner {
            _id
          }
